...
static GPIO_PORT GPIO_PORTS[] =
{
//        { &RCC_AHB1ENR, &RCC_AHB1RSTR, RCC_AHB1ENR_DMA1EN,  },
			//&GPIOA_MODER, &GPIOA_OTYPER, &GPIOA_OSPEEDR, &GPIOA_PUPDR, &GPIOA_IDR,
			//&GPIOA_ODR, &GPIOA_BSRR, &GPIOA_LCKR, &GPIOA_AFRL, &GPIOA_AFRH

		{ GPIOA_MODER, { NULL, NULL, 1} },

};


#if 0
GPIO_PORT* GPIO_PORT::GetInstance (GPIO_PORT_N portN)
{
    switch (portN)
    {
    case PORTA:
        return GPIOA::GetInstance();
    case PORTB:
        return GPIOB::GetInstance();
    case PORTC:
        return GPIOC::GetInstance();
    case PORTD:
        return GPIOD::GetInstance();
    case PORTE:
        return GPIOE::GetInstance();
    case PORTF:
        return GPIOF::GetInstance();
    case PORTG:
        return GPIOG::GetInstance();
    case PORTH:
        return GPIOH::GetInstance();
