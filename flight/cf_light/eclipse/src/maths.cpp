...
void buildRotationMatrix(fp_angles_t *delta, float matrix[3][3])
{
    ...
    matrix[0][2] = siny;
    matrix[1][0] = sinzcosx + (coszsinx * siny);
    matrix[1][1] = coszcosx - (sinzsinx * siny);
    matrix[1][2] = -sinx * cosy;
    matrix[2][0] = (sinzsinx) - (coszcosx * siny);
    matrix[2][1] = (coszsinx) + (sinzcosx * siny);
...
void rotateV(struct fp_vector *v, fp_angles_t *delta)
{
    ...
    v->X = v_tmp.X * matrix[0][0] + v_tmp.Y * matrix[1][0] + v_tmp.Z * matrix[2][0];
    v->Y = v_tmp.X * matrix[0][1] + v_tmp.Y * matrix[1][1] + v_tmp.Z * matrix[2][1];
    v->Z = v_tmp.X * matrix[0][2] + v_tmp.Y * matrix[1][2] + v_tmp.Z * matrix[2][2];
}
