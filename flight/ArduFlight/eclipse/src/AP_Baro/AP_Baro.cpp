...
void AP_Baro::calibrate()
{
    ...
    for (uint8_t i = 0; i < 10; i++) {
        ...
        do {
            ...
            if (millis() - tstart > 500) {
                panic("PANIC: AP_Baro::read unsuccessful "
    ...
    for (uint8_t i=0; i<_num_sensors; i++) {
        if (count[i] == 0) {
            ...
            sensors[i].ground_temperature = (sum_temperature[i] / count[i]);
        }
    }
    ...
    panic("AP_Baro: all sensors uncalibrated");
}
...
void AP_Baro::update_calibration()
{
    for (uint8_t i=0; i<_num_sensors; i++) {
        ...
        sensors[i].ground_temperature = (get_calibration_temperature(i));

        // don't notify the GCS too rapidly or we flood the link
        uint32_t now = millis();
        if (now - _last_notify_ms > 10000) {
            //TODO notify ground station
...
void AP_Baro::set_external_temperature(float temperature)
{
    ...
    _last_external_temperature_ms = millis();
}
...
float AP_Baro::get_calibration_temperature(uint8_t instance) const
{
    ...
    if (_last_external_temperature_ms != 0 && millis() - _last_external_temperature_ms < 10000) {
...
void AP_Baro::init(void)
{
    ...
    //TODO get I2c and addr for baro
