...
RC_Channel::calc_pwm(void)
{
    ...
    _radio_out = constrain_int16(_radio_out, _radio_min, _radio_max);
}
...
RC_Channel::get_control_mid() const {
    if (_type_in == RC_CHANNEL_TYPE_RANGE) {
        int16_t r_in = (_radio_min+_radio_max)/2;
...
{
    _radio_min = MIN(_radio_min, _radio_in);
    _radio_max = MAX(_radio_max, _radio_in);
}
...
{
    int16_t r_in = constrain_int16(_radio_in, _radio_min, _radio_max);
