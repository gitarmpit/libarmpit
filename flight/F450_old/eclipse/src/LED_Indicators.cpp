...
{
    _rgb[0] = GPIO_Helper::GetPin(RGB_LED_RED);
    _rgb[0]->SetupGPIO_OutPP();

    _rgb[1] = GPIO_Helper::GetPin(RGB_LED_GREEN);
   _rgb[1]->SetupGPIO_OutPP();

    _rgb[2] = GPIO_Helper::GetPin(RGB_LED_BLUE);
    _rgb[2]->SetupGPIO_OutPP();

    RGB_Off();


    _greenMotorLed = GPIO_Helper::GetPin(LED_GREEN);
