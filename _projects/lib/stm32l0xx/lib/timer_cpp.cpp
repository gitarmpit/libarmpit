...
void TIM21_IRQHandler(void) {
  if (LL_TIM_IsActiveFlag_UPDATE(TIM21) == 1) {
    ...
    TIMER21::GetInstance()->HandleInterrupt();
  }
}
...
void TIM22_IRQHandler(void) {
  if (LL_TIM_IsActiveFlag_UPDATE(TIM22) == 1) {
    ...
    TIMER22::GetInstance()->HandleInterrupt();
  }
}
...
void LPTIM1_IRQHandler(void) {
  if (LPTIM1->IER & LPTIM_IER_ARRMIE) {
    if (LPTIM1->ISR & LPTIM_ISR_ARRM) {
      ...
      //if (__lptim1_handler) {
      //  __lptim1_handler(lptim1_ctx);
      //}
      LPTIMER::GetInstance()->HandleInterrupt();
    }
  }
}
...
/////////////////////////////////////////////////////////////////////
void LPTIMER::CalculateTimerValues (uint32_t us, uint8_t* presc, uint16_t* arr) {
    LL_RCC_ClocksTypeDef RCC_Clocks;
    LL_RCC_GetSystemClocksFreq(&RCC_Clocks);
...
void LPTIMER::SetupCounter(uint32_t period_us) {
  ...
  LPTIMER::CalculateTimerValues(period_us, &presc, &arr);

  LPTIM1->CFGR |= (presc << LPTIM_CFGR_PRESC_Pos);
  LPTIM1->ARR = arr;
...
void LPTIMER::SetupPWM(uint32_t period_us, uint32_t ds_us) {
  LL_APB1_GRP1_EnableClock(LL_APB1_GRP1_PERIPH_LPTIM1);
  LL_LPTIM_Enable(LPTIM1);
  LPTIM1->CFGR |= LPTIM_CFGR_PRELOAD;

  if (period_us != 0 && ds_us != 0) {
    uint8_t presc;
    uint16_t arr;
    ...
    uint16_t duty_count = (uint16_t)((float)arr * (1.0 - (float)ds_us / (float)period_us));

    LPTIM1->CMP = duty_count;
  }
}

void LPTIMER::UpdatePeriodDs(uint32_t period_us, uint32_t ds_us) {
  uint8_t presc;
  uint16_t arr;
