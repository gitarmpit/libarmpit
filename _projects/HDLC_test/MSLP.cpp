...
void MSLP::SendData(const uint8_t* data, uint16_t dataSize) {
  while (dataSize > 0) {
...
void MSLP::MakeFrame (uint8_t flags, const uint8_t* data, uint16_t dataSize) {
  init_HDLC(0);
...
void MSLP::SendFrame(uint8_t flags, const uint8_t* data, uint16_t dataSize) {
    ...
    if (bytesRead) {
      ...
      if (complete) {
        ...
        printf ("seq: %x, ack: %x, data_off: %d, flags: %x, window: %x, cs: %x, data_size: %d\n",
          _frame->hdr.seq, _frame->hdr.ack, _frame->hdr.data_off, _frame->hdr.flags, _frame->hdr.window,
          _frame->hdr.cs, _frame->hdr.data_size);
        _frame->hdr.cs = 0;
