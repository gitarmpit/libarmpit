...
#elif defined(STM32F2) || defined(STM32F4) || defined(STM32F10X_XL)
...
void TIMER_With_SlaveMode::HandleInterrupt()
{
    bool isUIF = IsUIF();
    bool isTIF = IsTIF();
    if (isUIF || isTIF)
    ...
    for (uint8_t i = 0; i < _nchannels; ++i)
    {
        if (_channels[i].IsCCFlag())
        {
            ...
            if (i == 0 && _pwmCaptureHandler)
            {
                ...
                _pwmCaptureHandler->Handle(period_us, duty_us);
            }
            else if (_channels[i]._ccHandler)
            {
                _channels[i]._ccHandler->HandleInterrupt(&_channels[i]);
            }
        }
...
TIMER_With_SlaveMode::TIMER_With_SlaveMode(uint32_t timx_base_addr, uint8_t nchannels, IRQn_Type irqNo,
        volatile uint32_t* rccEnableReg,  volatile uint32_t* rccResetReg, uint32_t rccBit, bool isAPB1)
: TIMER_With_Channels (timx_base_addr, nchannels, irqNo, rccEnableReg, rccResetReg, rccBit, isAPB1)
{
    _smcr = (volatile uint32_t*)(timx_base_addr + TIM_SMCR_OFFSET);
    _pwmCaptureHandler = 0;
}
...
void TIMER_Channel::SetOCCPolarityLow (bool set)
{
    if (set)
...
void TIMER_Channel::SetICCRisingEdge ()
{
    *_ccer &= ~(TIM_CCER_CC1P<<(_channelNo-1)*4);
}
void TIMER_Channel::SetICCFallingEdge ()
{
    *_ccer |= (TIM_CCER_CC1P<<(_channelNo-1)*4);
}

#ifdef STM32F4
void TIMER_Channel::SetICCBothEdges ()
{
    *_ccer |= (TIM_CCER_CC1P<<(_channelNo-1)*4);
    *_ccer |= (TIM_CCER_CC1NP<<(_channelNo-1)*4);
...
TIMER_With_SlaveMode(timx_base_addr, 4, updateIrqNo, rccEnableReg, rccResetReg, rccBit,  false),
...
void TIMER::SetInterruptHandler (TIMER_Interrupt_Handler* handler)
{
    _irqHandlers[0] = handler;
...
uint32_t TIMER::GetCounterValue()
{
    return *_cnt;
}

uint32_t TIMER::GetPrescalerValue()
{
    return *_psc;
}

uint32_t TIMER::GetAutoReloadValue()
{
    return *_arr;
}
...
void TIMER::CalculateTimerValues_ns (uint32_t ns, uint32_t* count, uint16_t* presc)
{
    uint32_t timClk = RCC_GetTIMx_CLK(_isAPB1);
    uint64_t count64 = (uint64_t)timClk  * (uint64_t)ns / 1000000000llu;
...
uint32_t TIMER::GetTimerClockFreq()
{
    return RCC_GetTIMx_CLK(_isAPB1);
}
...
void TIMER::SetUpdatePeriod_ns (uint32_t ns)
{
    uint32_t count;
    uint16_t presc;
...
void TIMER_Channel::UpdatePWMDuty(uint32_t duty_us) 
{
    ...
    uint16_t duty_count = (uint64_t)duty_us * (uint64_t)_timer->GetTimerClockFreq() / 1000000ull /
                          (uint64_t)(_timer->GetPrescalerValue() + 1); 

    SetCCR((uint16_t)duty_count);
}

void TIMER_Channel::UpdatePWM(uint32_t period_us, uint32_t duty_us)
{
...
void TIMER_With_SlaveMode::CaptureInputPWM (uint32_t max_period_us, PWM_Capture_Handler* handler)
{
    uint32_t period_count;
    uint16_t presc = 1;
    ...
    CalculateTimerValues (max_period_us, &period_count, &presc);
    SetPrescalerValue(presc-1);
    SetCounterValue(0);
    
    _pwmCaptureHandler = handler;

    TIMER_Channel* ch1 = GetChannel(1);

    ch1->SetModeInputDirect();
    ch1->SetOCCPolarityLow(false);
    ch1->EnableCCInterrupt(true);

    TIMER_Channel* ch2 = GetChannel(2);
    ch2->SetModeInputInverse();
    ch2->SetOCCPolarityLow(true);

    SetSlaveModeTrigger_TI1FP1();
    SetSlaveMode_Reset();

    *(ch1->_ccer) |= (TIM_CCER_CC1E | TIM_CCER_CC2E);

    EnableNVICInterrupt(true);
    EnableCounter(true);
