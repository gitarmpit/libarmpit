...
static uint8_t yAdvance;  //can override the font associated value
static uint8_t xAdvance;

//ignored for custom fonts;
static int textsize_x;
static int textsize_y;

static void(*_drawPixel)(int16_t,int16_t,uint16_t);

//Color palette for 256 color bitmap (microsoft Paint)
static const uint16_t pal256[] =
{
...
static const uint16_t pal16[] = { 0x0000, 0x8000, 0x0400, 0x8400, 0x0010, 0x8010, 0x0410, 0x8410, 0xc618, 0xf800, 0x07e0, 0xffe0, 0x001f, 0xf81f, 0x07ff, 0xffff };

static void GFX_drawPixel(int16_t x, int16_t y, uint16_t color);
static uint8_t GFX_printf_va(const char *fmt, va_list args);
static void GFX_writePixel(int16_t x, int16_t y, uint16_t color);

static void GFX_startWrite();
static void GFX_endWrite();
static void GFX_drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color);
static void GFX_drawFastHLine(int16_t x, int16_t y, int16_t h, uint16_t color);
static void GFX_drawCircleHelper(int16_t x0, int16_t y0, int16_t r, uint8_t cornername, uint16_t color);
static void GFX_fillCircleHelper(int16_t x0, int16_t y0, int16_t r, uint8_t cornername, int16_t delta, uint16_t color);
static void GFX_drawChar(int16_t x, int16_t y, uint16_t c, uint16_t color, uint16_t bg);
static void GFX_writeChar(uint16_t c);
...
uint8_t GFX_printf(const char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
    uint8_t size = GFX_printf_va(fmt, args);
    va_end(args);
    return size;
}
...
uint8_t GFX_printf_xy(uint8_t x, uint8_t y, const char *fmt, ...)
{
    ...
    va_list args;
    va_start(args, fmt);
    int size = GFX_printf_va(fmt, args);
    va_end(args);
    return size;
}
...
void GFX_drawBmp(int16_t x, int16_t y, const uint8_t *bmp)
{
    ...
    {
        GFX_printf_xy(0, 0, "Not supported color depth: %d", hdr2->BitsPerPixel);
        return;
    }
    ...
    for (uint32_t j = 0; j < h; ++j)
    {
        ...
        for (uint32_t i = 0; i < wread; ++i)
        {
            if (hdr2->BitsPerPixel == 4)
            {
                ...
                {
                    GFX_drawPixel(x + wtotal++, y + j, pal16[c1]);
                }
                if (wtotal <= hdr2->Width)
                {
                    GFX_drawPixel(x + wtotal++, y + j, pal16[c2]);
                }
            }
            else
            {
...
// (x,y) is leftmost point; if unsure, calling function
// should sort endpoints or call drawLine() instead
static void GFX_drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color)
{
    // Update in subclasses if desired!
    GFX_startWrite();
...
static void GFX_fillCircleHelper(int16_t x0, int16_t y0, int16_t r, uint8_t cornername, int16_t delta, uint16_t color)
{
    ...
    while (x < y)
    {
        ...
        if (cornername & 0x1)
        {
            GFX_writeFastVLine(x0 + x, y0 - y, 2 * y + 1 + delta, color);
            GFX_writeFastVLine(x0 + y, y0 - x, 2 * x + 1 + delta, color);
        }
        if (cornername & 0x2)
        {
            GFX_writeFastVLine(x0 - x, y0 - y, 2 * y + 1 + delta, color);
    ...
    { // 'Classic' built-in font
            ...
            for (int8_t j = 0; j < 8; j++, line >>= 1)
            {
                if (line & 1)
                {
                    ...
                    GFX_writePixel(x + i, y + j, color);
                        ...
                        GFX_writeFillRect(x + i * textsize_x, y + j * textsize_y,
        ...
        { // If opaque, draw vertical line for last column
            if (textsize_x == 1 && textsize_y == 1)
                ...
                GFX_writeFillRect(x + 5 * textsize_x, y, textsize_x, 8 * textsize_y, bg);
        }
        GFX_endWrite();
...
static void GFX_writeChar(uint16_t c)
{
    ...
    { // 'Classic' built-in font
        ...
        {                 // Ignore carriage returns
            ...
            GFX_drawChar(cursor_x*textsize_x*6, cursor_y*textsize_y*8, c, fgcolor, bgcolor);
        ...
        else if (c != '\r')
        {
            ...
            if ((c >= first) && (c <= gfxFont->last))
            {
                ...
                { // Is there an associated bitmap?
                    ...
                    GFX_drawChar(cursor_x, (cursor_y + 1) * yAdvance, c, fgcolor, bgcolor);
                }
