...
void Si4432::boot()
{
    ...
    //direct output
    ChangeRegister(REG_GPIO0_CONF, 0xA); 
    //ChangeRegister(REG_GPIO1_CONF, 0xA);
    //ChangeRegister(REG_GPIO2_CONF, 0xA);
...
//return: 0=packet sent, 1: retry 2:failed (max retry reached)
int Si4432::trySendPacket(uint8_t length, const uint8_t* data, uint16_t maxRetry)
{
    ...
    {
    	//not done, will re-check status on next call until maxRetry count
        in_progress++;
        status = 1;
    }

    //give up
    if (in_progress == maxRetry)
    {
        switchMode(Ready);
...
void Si4432::ToggleGPIO()
{
	uint8_t res = (ReadRegister(REG_IOPORT_CONF) & 0x1);
    ChangeRegister(REG_IOPORT_CONF, res ? 0x0 : 0x1);
}

void Si4432::SetGPIO() 
{
    ChangeRegister(REG_IOPORT_CONF, 0x1);
}

void Si4432::ResetGPIO() 
{
    ChangeRegister(REG_IOPORT_CONF, 0x0);
}
