...
RF22::RF22(SPI* spi, GPIO_PIN* ssPin, GPIO_PIN* sdnPin, GPIO_PIN* interruptPin)
{
    _spi = spi;
    _ssPin = ssPin;
    _sdnPin = sdnPin;
...
void RF22::spiBurstRead(uint8_t startReg, uint8_t* data, uint8_t len)
{
    ...
    {
        data[i] = _spi->TransmitByte(0xFF);
    }

    _ssPin->Set();
}

void RF22::spiBurstWrite(uint8_t startReg, const uint8_t* data, uint8_t len)
{
