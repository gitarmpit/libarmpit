    ...
    if(!(bit >>= 1)) {       // Advance to next bit, end of byte reached?
        if(!firstCall) { // Format output table nicely
            if(++row >= 12) {        // Last entry on line?
                ...
                fprintf(f_out, ", ");    //   Simple comma delim
            }
        }
        fprintf(f_out, "0x%02X", sum); // Write byte value
...
int main(int argc, char *argv[]) 
{
    ...
    */
    ...
    if((!(fontName = malloc(strlen(ptr) + 20))) || (!(table = (GFXglyph *)malloc((last - first + 1) * sizeof(GFXglyph))))) 
    {
    ...
    ptr = fontName;
    //sprintf (fontName, "%s%d_%d", ptr, size, dpi);

    char fname[256];
    sprintf (fname, "%s%d.h", fontName, size); 
    ...
    fprintf(f_out, "const uint8_t %sBitmaps[] = {\n  ", fontName);
