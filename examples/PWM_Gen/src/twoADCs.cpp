...
void TwoADCs() {
	...
	MyButtonHandler2 buttonHandler(tim3);
	...
	while (1) {
		...
		//period_us = 2*scale + (int) (((float) (val + val2)) * (2*scale*100 - 2*scale) / 8192);

		// int period_us = 1000 + (int) (((float) (val + val2)) * 4.096 );

//		if (val0 == -1 || abs((int) (val - val0)) > 0 ||
//			abs((int) (val2 - val2_0)) > 0 ||
//			_scale != scale0 )
		if (period != period0)
		{
