...
AP_InertialSensor::AP_InertialSensor() :
    ...
    //_dataflash(NULL),  TODO
    _accel_cal_requires_reboot(false),
    _startup_error_counts_set(false),
    _startup_ms(0)
{
    if (_s_instance) {
        panic("Too many inertial sensors");
    }
    _s_instance = this;
    //AP_Param::setup_object_defaults(this, var_info);  TODO
...

{
    ...
    if (_gyro_count == 0 || _accel_count == 0) {
        panic("INS needs at least 1 gyro and 1 accel");
    }
}
...
AP_InertialSensor::detect_backends(void)
{
    ...
    _add_backend(AP_InertialSensor_MPU9250::probe(*this));  //TODO 2nd parameter: I2C dev



    if (_backend_count == 0) {
        panic("No INS backends available");
    }

    // set the product ID to the ID of the first backend
    _product_id = (_backends[0]->product_id());
}
...
void AP_InertialSensor::update(void)
{
    ...
    if (!_hil_mode) {
        ...
        if (!_startup_error_counts_set) {
            ...
            if (_startup_ms == 0) {
                _startup_ms = millis();
            } else if (millis()-_startup_ms > 2000) {
...
void AP_InertialSensor::wait_for_sample(void)
{
    ...
    if (_next_sample_usec - now <=_sample_period_usec) {
        ...
        if (now2+100 < _next_sample_usec) {
            //timing_printf("shortsleep %u\n", (unsigned)(_next_sample_usec-now2));  //TODO
        }
        ...
        //timing_printf("overshoot1 %u\n", (unsigned)(now-_next_sample_usec));  //TODO
        _next_sample_usec += _sample_period_usec;
    } else {
...
void AP_InertialSensor::_acal_save_calibrations()
{
    ...
    for (uint8_t i=0; i<_accel_count; i++) {
        if (_accel_calibrator[i].get_status() == ACCEL_CAL_SUCCESS) {
            ...
            _accel_offset[i] = bias;
            _accel_scale[i] = gain;

            //TODO save offset and scale
            ...
            _accel_offset[i] = (Vector3f(0,0,0));
            _accel_scale[i] = (Vector3f(0,0,0));
        }
    }
