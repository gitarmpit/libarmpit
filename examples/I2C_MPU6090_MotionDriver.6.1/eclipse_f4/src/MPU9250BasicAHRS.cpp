...
#define SerialDebug 0   // set to true to get Serial output for debugging
...
static int16_t tempCount;      // temperature raw count output
...
static void writeByte(uint8_t slave_address, uint8_t reg_address, uint8_t data)
{
    _i2c->MasterWrite(slave_address, reg_address, data);
}

static uint8_t readByte(uint8_t slave_address, uint8_t reg_address)
{
    uint8_t data;
...
static void readBytes(uint8_t slave_address, uint8_t reg_address, uint8_t count,
        uint8_t *data)
{
...
//        lcd->clearDisplay();
//        lcd->write_float(0, 0, ax, 2);
//        lcd->write_float(0, 1, ay, 2);
//        lcd->write_float(0, 2, az, 2);
...
//        lcd->write_float(0, 3, atan2(my, mx)*180./PI, 1);
//      lcd->display();
...
//  MadgwickQuaternionUpdate(ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f,  my,  mx, mz);
    ...
    else
    {
        ...
        { // update LCD once per half-second independent of read rate
            ...
            yaw -= 4.39; // Declination at Indianapolis, IN
            roll *= 180.0f / PI;
            ...
            float rateHz = (float) sumCount / sum;
            lcd->clearDisplay();
            lcd->printf(2, 0, 0, "%6.2f", yaw);
            lcd->printf(2, 0, 1, "%6.2f", pitch);
            lcd->printf(2, 0, 2, "%6.2f", roll);
            lcd->printf(2, 0, 3, "%6.2f", rateHz);
            lcd->display();
...
static short gyro[3], a[3], c[3], sensors;
static long  qq[4];
static uint32_t ts;
...
static void loop2(Adafruit_Nokia5110* lcd)
{
    ...
    if (readByte(MPU9250_ADDRESS, INT_STATUS) & 0x01)
    {
        ...
        int rc = dmp_read_fifo (gyro, a, qq, &ts, &sensors, &more);
        if (rc == 0)
        {

            ax = (float) a[0] * aRes; // - accelBias[0];  // get actual g value, this depends on scale being set
            ay = (float) a[1] * aRes; // - accelBias[1];
            az = (float) a[2] * aRes; // - accelBias[2];

            getGres();

            gx = (float) gyro[0] * gRes; // get actual gyro value, this depends on scale being set
...
void basic_AHRS2(Adafruit_Nokia5110* lcd)
{
    ...
    //volatile uint8_t d = readByte(AK8963_ADDRESS, WHO_AM_I_AK8963); // Read WHO_AM_I register for AK8963
