...
sub pass1
{
    ...
    my $table_start = "core peripheral register regions\\s+\$";
    my $table_found     = 0;
    my @pnames = ();
    ...
    foreach (@lines)
    {
        ...
        if ( !$table_found && /core peripheral register regions\s*$/i )
        {
            $table_found = 1;
            next;
        }
        ...
        if ($table_found && /^\s*(0x(?:[0-9A-F]){4}\s*(?:[0-9A-F]){4})\s*-\s*0x(?:[0-9A-F]){4}\s*(?:[0-9A-F]){4}\s+(.*)/i )
        {
            my $addr = $1;
            my $d    = $2;
